...
<script>
    export default {
        ...
        //  不在vuex中写异步action   这里也相当于异步action  在main.js全局引入api/index.js  并将其设为实例对象的方法
        methods: {
            async getMyOrder(page) {
                this.currentPage = page
                // 调用接口请求函数  
                const result = await this.$API.reqMyOrder(page, this.pageSize)
                // 判断
                if (result.code === 200) {
                    // 获取数据
                    const {
